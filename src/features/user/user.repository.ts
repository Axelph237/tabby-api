...
class UserRepository extends Repository<typeof usersTable> {
    index(projection?: UsersProjection) {
        return projSelect<typeof usersTable>(projection).from(usersTable).$dynamic();
    }

    get(id: UUID, projection?: UsersProjection) {
        return projSelect<typeof usersTable>(projection).from(usersTable).where(eq(usersTable.id, id)).$dynamic();
    }
