...
import { asGuest, asGuestTransaction, asUser } from '@config/drizzle/query-wrappers'
...
export const orderRoutes = new Elysia({ prefix: "/orders/:sessId" })
	...
	.get("/", ({ orderRepo, user }) => 
		asUser(user?.id, orderRepo.index()), 
		{
			isAuthenticated: true
		})
	// 3.2 - Add an order to a session
	.post("/", ({ body, orderRepo }) => 
		asGuestTransaction(orderRepo.$place(body)),
		{
	...
	.delete("/:orderId", ({ params, user, orderRepo }) => 
		asUser(user?.id, orderRepo.$setStatus(params.orderId, "canceled")), 
		{
			params: t.Object({
				orderId: t.Integer()
			}),
